...
type AssetEditFormState = {
  brand: string;
  categoryName: string;
  domainCode: AssetEditRecord["domain"]["code"];
  imageRef: string;
  legacyFg: string;
  legacyQty: string;
  location: string;
...
function Input(
  ...
  return (
    <input
      {...props}
      className={[
        "h-11 rounded-md border border-border bg-white px-3 text-sm font-semibold text-ink outline-none ring-brand-accent/20 focus:ring-4",
        props.className,
      ].filter(Boolean).join(" ")}
    />
  );
...
function Textarea(
  ...
  return (
    <textarea
      {...props}
      className={[
        "min-h-32 rounded-md border border-border bg-white px-3 py-3 text-sm font-semibold text-ink outline-none ring-brand-accent/20 focus:ring-4",
        props.className,
      ].filter(Boolean).join(" ")}
    />
  );
...
export function AssetEditForm({
  ...
  const [form, setForm] = useState<AssetEditFormState>(() => toFormState(asset));
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <h1 className="mt-3 text-3xl font-bold text-navy">Edit {asset.assetModel.name}</h1>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <Field label="Domain">
              <Select
                ...
                onChange={(event) =>
                  setField("domainCode", event.target.value as AssetEditRecord["domain"]["code"])
                }
      ...
      <section className="rounded-md border border-border bg-white p-5 shadow-sm">
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <Field label="Current Status">
            <Select
              onChange={(event) =>
                setField("status", event.target.value as AssetEditRecord["status"])
              }
              value={form.status}
            >
        ...
        <datalist id="asset-location-options">
          {options.locations.map((location) => (
            <option key={`${location.code ?? "none"}-${location.name}`} value={location.name} />
          ))}
        </datalist>
