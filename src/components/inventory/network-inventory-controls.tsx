...
function buildHref(
  filters: NetworkInventoryControlFilters,
  overrides: Partial<NetworkInventoryControlFilters>,
) {
  ...
  return `/dashboard/network${params.size ? `?${params.toString()}` : ""}`;
}

function HiddenFilters({ filters }: { filters: NetworkInventoryControlFilters }) {
  return (
    <>
...
function FilterDrawer({
  ...
  total,
  types,
}: NetworkInventoryControlsProps & { isOpen: boolean; onClose: () => void }) {
  if (!isOpen) {
    return null;
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-black/30">
      ...
      <aside className="h-full w-full max-w-sm overflow-y-auto bg-white p-6 shadow-2xl">
        ...
        <form action="/dashboard/network" className="mt-6" method="get">
